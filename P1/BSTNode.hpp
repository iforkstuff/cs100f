...
public:
  ...
  BSTNode<Data>* successor() {
    if(this == 0) return 0;
    BSTNode<Data> *pointer = this;
    if(pointer->right != 0) {
      pointer = pointer->right;
      while(pointer->left != 0) pointer = pointer->left;
      return pointer;
    }
    else {
      if(pointer->parent == 0) return 0;
      if(pointer == pointer->parent->left) return pointer->parent;
      else if(pointer == pointer->parent->right) {
	while(pointer->parent != 0) {
	  if(pointer->parent->data > pointer->data) break;
